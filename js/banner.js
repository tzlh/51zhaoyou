    ...
    a.fn.maiPlayer = function (c) {
        ...
        this.each(function () {
            ...
            function l(r, q) {
                if (r.is(":animated") == false) {
                    ...
                    if (c.showPage != -1 && c.showPage != h) {
                        r.animate({left: -c.showPage * i + "px"}, c.speedAnimate)
                    } else {
                        if (c.showPage == -1) {
                            c.showPage = h - 1;
                            r.css({left: -(i * (c.showPage - 1)) + "px"});
            ...
            a(p).hover(function () {
                o();
                if (c.textDisplay == 1) {
                    a(p).find(".bg:first").slidebannerUp(c.textSpeed);
                    a(p).find(".ul-text:first").slidebannerUp(c.textSpeed)
                } else {
                    if (c.textDisplay == 2) {
                        a(p).find(".bg:first").slidebannerDown(c.textSpeed);
                        a(p).find(".ul-text:first").slidebannerDown(c.textSpeed)
                    }
                }
            }, function () {
                if (c.autoPlay) {
...
$(document).ready(function () {
    $('#img-play').maiPlayer({speedSwitch: 5000, playerWidth: 679});
});